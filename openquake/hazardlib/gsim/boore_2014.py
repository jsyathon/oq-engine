...
def _get_inter_event_tau_adj(C, mag):
    """
    Returns the inter-event standard deviation (tau), which is dependent
    on magnitude
    """
    sc0 = np.full_like(mag, 0.7)
    sc1 = np.full_like(mag, -0.15)
    sc0[mag > 5] -= 0.25 * (np.minimum(mag[mag > 5] , 7) - 5)
    sc1[mag > 5] += 0.185 * (np.minimum(mag[mag > 5] , 7) - 5)
    tau = sc0 + sc1 * C["sig_f"]
    return tau


def _get_total_sigma_adj(C, mag):
    """
    Returns the total standard deviation (sigma), which is dependent
    on magnitude
    """
    sc0 = np.full_like(mag, 1.0)
    sc1 = np.full_like(mag, -0.16)
    sc0[mag > 5] -= 0.235 * (np.minimum(mag[mag > 5] , 7) - 5)
    sc1[mag > 5] += 0.17 * (np.minimum(mag[mag > 5] , 7) - 5)
    sigma = sc0 + sc1 * C["sig_f"]
    return sigma


def _get_s2s_adj(C, mag):
    """
    Returns the total standard deviation (sigma), which is dependent
    on magnitude
    """
    sc0 = np.full_like(mag, 0.5)
    sc1 = np.full_like(mag, 0.0)
    sc0[mag > 5] -= 0.06 * (np.minimum(mag[mag > 5] , 7) - 5)
    sc1[mag > 5] += 0.04 * (np.minimum(mag[mag > 5] , 7) - 5)
    s2s = sc0 + sc1 * C["sig_f"]
    return s2s
...
def _get_magnitude_scaling_term_adj(sof, C, ctx):
    """
    Returns the magnitude scling term defined in equation (2)
    """
    mag_term = _get_style_of_faulting_term_adj(C, ctx)
    if not sof:  # unspecified style-of-faulting
        return C["e1"] + C["e0"] + mag_term
    return mag_term


def _get_magnitude_scaling_term_adj_upper(C,ctx):
    """
    Returns the delta for BooreEtAl2014Adj upper branch
    """
    df = np.full_like(ctx.mag,0.1)
    dn = np.minimum(-0.15+0.05*(np.maximum(ctx.mag,4)-4),0)
    dl = np.maximum(0.35-0.05*(np.maximum(ctx.mag,4)-4),0.25) + C["dl_ut"]
    # delta = dl + df * np.maximum(np.log(ctx.rjb/70),0) + dn * np.minimum(np.log(ctx.rjb/10),0)
    delta = dl + df * np.maximum(np.log(np.maximum(ctx.rjb,0.1)/70),0) + dn * np.minimum(np.log(np.maximum(ctx.rjb,0.1)/10),0)
    return delta


def _get_magnitude_scaling_term_adj_lower(C,ctx):
    """
    Returns the delta for BooreEtAl2014Adj upper branch
    """
    df = np.full_like(ctx.mag,-0.1)
    dn = np.maximum(0.05-0.025*(np.maximum(ctx.mag,4)-4),0)
    dl = -np.maximum(0.35-0.05*(np.maximum(ctx.mag,4)-4),0.25) + C["dl_lt"]
    # delta = dl + df * np.maximum(np.log(ctx.rjb/70),0) + dn * np.minimum(np.log(ctx.rjb/10),0)
    delta = dl + df * np.maximum(np.log(np.maximum(ctx.rjb,0.1)/70),0) + dn * np.minimum(np.log(np.maximum(ctx.rjb,0.1)/10),0)
    return delta
...
def _get_stddevs_adj(kind, C, ctx):
    """
    Returns the aleatory uncertainty terms described in equations (13) to
    (17)
    """
    tau = _get_inter_event_tau_adj(C, ctx.mag)
    sigma = _get_total_sigma_adj(C, ctx.mag)
    s2s =  _get_s2s_adj(C, ctx.mag)
    phi = np.sqrt(sigma * sigma - tau * tau)
    ss2 = np.sqrt(sigma * sigma - s2s * s2s)
    return [ss2, tau, phi]
...
def _get_style_of_faulting_term_adj(C, ctx):
    """
    Get fault type dummy variables
    Fault type (Strike-slip, Normal, Thrust/reverse) is
    derived from rake angle.
    ...
    rake is always given.
    """
    res = np.zeros_like(ctx.mag)
    res[ctx.mag <= CONSTS["Mh1"]] = C["e11"] + (C["e4adj"] * (ctx.mag - CONSTS["Mh1"]))[ctx.mag <= CONSTS["Mh1"]]
    res[(ctx.mag <= CONSTS["Mh2"]) & (ctx.mag > CONSTS["Mh1"])] = C["e12"] + (C["e5adj"] * (ctx.mag - CONSTS["Mh2"]))[(ctx.mag <= CONSTS["Mh2"]) & (ctx.mag > CONSTS["Mh1"])]
    res[ctx.mag > CONSTS["Mh2"]] = C["e12"] + (C["e6"] * (ctx.mag - CONSTS["Mh2"]))[ctx.mag > CONSTS["Mh2"]]
    # strike slip
    res[(np.abs(ctx.rake) <= 30.) |
        ((180. - np.abs(ctx.rake)) <= 30.)] += 0.
    # normal
    res[(ctx.rake > -150.) & (ctx.rake < -30.)] += C["e3"] - C["e1"]
    # reverse
    res[(ctx.rake > 30.) & (ctx.rake < 150.)] += C["e3"] - C["e1"]
    # print(res)
    return res
...
class BooreEtAl2014Adj(BooreEtAl2014):
    """
    This class implements the Boore et al. (2014) model considering the
    correction to the path scaling term for High Q regions (e.g. China and
    Turkey)
    ...
    def compute(self, ctx: np.recarray, imts, mean, sig, tau, phi):
        ...
        for m, imt in enumerate(imts):
            ...
            pga_rock = _get_pga_on_rock(
                self.kind, self.region, self.sof, C_PGA, ctx)
            mean[m] = (
                _get_magnitude_scaling_term_adj(self.sof, C, ctx) +
                _get_path_scaling(self.kind, self.region, C, ctx) +
                _get_site_scaling(self.kind, self.region,
                                  C, pga_rock, ctx, imt, ctx.rjb,
                                  self.usgs_basin_scaling,
            ...
            if self.sigma_mu_epsilon:
                mean[m] += (self.sigma_mu_epsilon*get_epistemic_sigma(ctx))
            # sig[m], tau[m], phi[m] = _get_stddevs_adj(self.kind, C, ctx)
            sig[m], tau[m], phi[m] = _get_stddevs_adj(self.kind, C, ctx)

    COEFFS = CoeffsTable(sa_damping=5, table="""\
    IMT            e0          e1          e2          e3         e4          e5          e6         Mh          c1         c2          c3          h        Dc3           c            Vc          f4          f5          f6          f7           R1           R2        DfR        DfV         f1         f2       tau1       tau2         e11         e12       e4adj       e5adj       dl_ut       dl_lt       sig_f
    pgv      5.037000    5.078000    4.849000    5.033000   1.073000   -0.153600    0.225200   6.200000   -1.243000   0.148900   -0.004610   5.300000   0.000000   -0.840000   1300.000000   -0.100000   -0.008440   -9.900000   -9.900000   105.000000   272.000000   0.082000   0.080000   0.644000   0.552000   0.401000   0.346000    3.206940    5.161960    2.120760    0.977510    0.000000     0.00000     0.69897
    pga      0.447300    0.485600    0.245900    0.453900   1.431000    0.050530   -0.166200   5.500000   -1.134000   0.191700   -0.009570   4.500000   0.000000   -0.600000   1500.000000   -0.150000   -0.007010   -9.900000   -9.900000   110.000000   270.000000   0.100000   0.070000   0.695000   0.495000   0.398000   0.348000   -0.422740    0.338250    1.585260    0.380490    0.000000     0.00000     0.30103
    8.000   -2.686100   -2.582900   -2.943800   -2.782300   2.061300   -0.103820    1.023200   6.200000   -1.268800   0.131460    0.000000   9.570000   0.000000   -0.750330    760.810000   -0.000117   -0.001370    1.308400    0.795380   130.870000   245.230000   0.059000   0.000000   0.510000   0.630000   0.507000   0.278000   -5.966710   -2.177900    3.018800    1.894400    0.077000    -0.27300     1.00000
    8.500   -2.782000   -2.675200   -3.059700   -2.877600   2.008400   -0.121140    1.033500   6.200000   -1.283900   0.137420    0.000000   9.620000   0.000000   -0.725440    764.500000   -0.000131   -0.001370    1.282200    0.776710   130.710000   238.130000   0.059000   0.000000   0.509000   0.622000   0.503000   0.265000   -6.033523   -2.290545    3.045010    1.871485    0.082500    -0.29250     1.00000
    9.000   -2.879200   -2.768700   -3.171300   -2.975900   1.960500   -0.134070    1.045300   6.200000   -1.298900   0.142940    0.000000   9.660000   0.000000   -0.701610    768.070000   -0.000108   -0.001370    1.251500    0.754250   130.500000   229.560000   0.060000   0.000000   0.509000   0.613000   0.498000   0.252000   -6.100335   -2.403190    3.071220    1.848570    0.088000    -0.31200     1.00000
    9.500   -2.976900   -2.863400   -3.278500   -3.076000   1.918900   -0.143640    1.056700   6.200000   -1.313000   0.147810    0.000000   9.660000   0.000000   -0.678500    771.550000   -0.000060   -0.001360    1.217900    0.729250   130.260000   220.020000   0.060000   0.000000   0.509000   0.604000   0.492000   0.239000   -6.167148   -2.515835    3.097430    1.825655    0.093500    -0.33150     1.00000
    10.00   -3.070200   -2.953700   -3.377600   -3.172600   1.883700   -0.150960    1.065100   6.200000   -1.325300   0.151830    0.000000   9.660000   0.000000   -0.655750    775.000000    0.000000   -0.001360    1.182900    0.703000   130.000000   210.000000   0.060000   0.000000   0.510000   0.604000   0.487000   0.239000   -6.233960   -2.628480    3.123640    1.802740    0.099000    -0.35100     1.00000
    """)


class BooreEtAl2014AdjUpper(BooreEtAl2014Adj):
    """
    This class implements the Boore et al. (2014) model considering the
    correction to the path scaling term for High Q regions (e.g. China and
    Turkey)
    ...
    def __init__(self, region='nobasin', sof=True, sigma_mu_epsilon=1.0,
                 usgs_basin_scaling=False, cybershake_basin_adj=False):
        self.region = region
        self.sof = sof
        self.sigma_mu_epsilon = sigma_mu_epsilon
    ...
    def compute(self, ctx: np.recarray, imts, mean, sig, tau, phi):
        ...
        for m, imt in enumerate(imts):
            ...
            if self.sigma_mu_epsilon:
                mean[m] += (self.sigma_mu_epsilon*_get_magnitude_scaling_term_adj_upper(C,ctx))
            sig[m], tau[m], phi[m] = _get_stddevs_adj(self.kind, C, ctx)


class BooreEtAl2014AdjLower(BooreEtAl2014Adj):
    """
    This class implements the Boore et al. (2014) model considering the
    correction to the path scaling term for High Q regions (e.g. China and
    Turkey)
    ...
    def compute(self, ctx: np.recarray, imts, mean, sig, tau, phi):
        ...
        for m, imt in enumerate(imts):
            ...
            if self.sigma_mu_epsilon:
                mean[m] += (self.sigma_mu_epsilon*_get_magnitude_scaling_term_adj_lower(C,ctx))
            sig[m], tau[m], phi[m] = _get_stddevs_adj(self.kind, C, ctx)
